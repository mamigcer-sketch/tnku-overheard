...
import { Menu, X, Bookmark, ShieldAlert, BookOpen, ExternalLink, Download, User, MessageCircle, Trophy } from 'lucide-react';
import Link from 'next/link';

export default function MobileMenu({ userUuid }: { userUuid?: string }) {
  const [isOpen, setIsOpen] = useState(false);
  
  // 🔥 MENÜYE CHAT VE SEFİRLER KISAYOLLARI EKLENDİ
  const menuItems = [
    ...
    <div className="relative z-50">
      ...
      {isOpen && (
        <>
          ...
          {/* Menü Kutusu - Yeni Cam Efektli (Glassmorphism) Tasarım */}
          <div className="absolute top-14 right-0 w-[260px] bg-[#0A0A0A]/95 backdrop-blur-3xl border border-white/10 rounded-[24px] p-2.5 shadow-[0_10px_50px_rgba(0,0,0,0.7)] z-50 animate-in fade-in zoom-in-95 duration-200">
            <div className="space-y-1">
              ...
              {/* 🔥 Dikkat Çekici İndirme Butonu */}
              <button 
                onClick={() => {
                  setIsOpen(false); 
                  window.dispatchEvent(new Event('trigger-install-modal'));
                }}
                className="w-full flex items-center justify-center gap-2 p-3.5 rounded-xl hover:bg-emerald-500/20 text-emerald-400 font-bold text-[14px] transition-all border border-emerald-500/30 bg-emerald-500/10 cursor-pointer active:scale-95"
              >
