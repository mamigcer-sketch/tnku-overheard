...
export default function EditableAvatar({ userUuid, currentAvatar, displayNickname, isOwnProfile }: any) {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > 5 * 1024 * 1024) {
      alert("Fotoğraf 5 MB'den büyük! Lütfen daha düşük boyutlu bir şey seç.");
      e.target.value = ''; 
      return;
    }
      ...
      <div className="bg-white dark:bg-[#0A0A0A] w-full max-w-sm rounded-[32px] p-6 relative z-10 animate-in fade-in zoom-in-95 duration-200 border border-gray-200 dark:border-white/10 shadow-2xl">
        ...
        <div className="grid grid-cols-6 gap-2 mb-6 bg-gray-50 dark:bg-[#050505] p-3 rounded-2xl border border-gray-200 dark:border-white/5">
          {PRESET_AVATARS.map((emoji) => (
            <button 
              key={emoji} disabled={isLoading} onClick={() => handleSelect(emoji)}
        ...
        <div className="relative">
          <input type="file" accept="image/jpeg, image/png, image/webp" className="absolute inset-0 w-full h-full opacity-0 cursor-pointer disabled:cursor-not-allowed" onChange={handleFileUpload} disabled={isLoading} />
          <button disabled={isLoading} className={`w-full py-4 rounded-2xl font-black text-[14px] flex items-center justify-center gap-2 transition-all shadow-lg ${isLoading ? 'bg-[#4DA3FF] text-white cursor-wait' : 'bg-gray-100 dark:bg-white/[0.05] text-gray-900 dark:text-white border border-gray-200 dark:border-white/10 active:scale-95'}`}>
  ...
  return (
    <>
      <div className="relative group cursor-pointer" onClick={() => isOwnProfile && setIsOpen(true)}>
        ...
        {isOwnProfile && (
          <div className="absolute -bottom-1 -right-1 w-8 h-8 bg-[#4DA3FF] rounded-full border-[3px] border-white dark:border-[#0A0A0A] flex items-center justify-center text-white shadow-md">
            <Pencil size={14} className="fill-white" />
          </div>
        )}
      </div>
      
      {/* İŞTE SİHİR BURADA ÇALIŞACAK */}
