...
export default async function ProfilePage({ params, searchParams }: { params: any, searchParams: any }) {
  ...
  return (
    // 🔥 DİNAMİK METİN RENGİ VE GEÇİŞLER
    <main className="min-h-screen text-gray-900 dark:text-white relative z-0 pb-20 selection:bg-[#4DA3FF]/30 transition-colors duration-300">
      ...
      <header className="sticky top-0 z-50 bg-white/80 dark:bg-[#050505]/80 backdrop-blur-3xl border-b border-gray-200 dark:border-white/[0.05] shadow-sm px-4 py-3 flex items-center justify-between transition-colors duration-300">
        ...
        <h1 className="text-[15px] font-black tracking-widest uppercase text-gray-900 dark:text-white flex items-center gap-1.5 transition-colors">
      ...
      <div className="max-w-2xl mx-auto pt-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <div className="px-4 pb-2">
          <div className="flex flex-col sm:flex-row items-center gap-5 sm:gap-6 bg-white dark:bg-white/[0.02] border border-gray-200 dark:border-white/[0.05] rounded-[32px] p-5 backdrop-blur-xl shadow-sm dark:shadow-[0_10px_40px_rgba(0,0,0,0.5)] transition-colors duration-300">
            ...
            {/* İSTATİSTİK KUTUSU (GÜNDÜZ/GECE) */}
            <div className="w-full flex-1 flex justify-around items-center bg-gray-50 dark:bg-black/40 border border-gray-200 dark:border-white/5 rounded-2xl py-3 px-2 shadow-inner transition-colors duration-300">
              <div className="flex flex-col items-center">
                <span className="text-[20px] font-black text-gray-900 dark:text-white">{postCount}</span>
              ...
              <div className="flex flex-col items-center">
                <span className="text-[20px] font-black text-gray-900 dark:text-white">{totalLikes}</span>
                <span className="text-[11px] font-bold text-gray-500 uppercase tracking-wider">Beğeni</span>
              </div>
              <div className="w-px h-8 bg-gray-200 dark:bg-white/10"></div>
              <div className="flex flex-col items-center">
                <span className="text-[20px] font-black text-gray-900 dark:text-white">{commentCount}</span>
          ...
          <div className="mt-5 px-2">
            <h2 className="text-[16px] font-black text-gray-900 dark:text-white flex items-center gap-2">
              {displayNickname}
              {userBadge && (
                <span className="bg-amber-100 text-amber-600 border border-amber-200 dark:bg-amber-500/10 dark:text-amber-500 dark:border-amber-500/20 text-[10px] px-2 py-0.5 rounded-md uppercase font-black tracking-widest shadow-sm">
                  {userBadge}
                </span>
              )}
            </h2>
            <p className="text-[12px] font-medium text-gray-500 dark:text-gray-400 mt-1 uppercase tracking-widest">TNKUOVERHEARD TAKİPÇİSİ</p>
            
            {/* OYUN TARZI SEVİYE BARI */}
            <div className="mt-4 flex items-center gap-3 w-full bg-white dark:bg-white/[0.03] border border-gray-200 dark:border-white/5 p-2.5 rounded-xl shadow-sm dark:shadow-none transition-colors duration-300">
              <span className="text-[12px] font-black text-amber-600 dark:text-amber-500 shrink-0 flex items-center gap-1.5 w-[70px]">
                <Flame size={14} className="animate-pulse" /> Lvl {level}
              </span>
              <div className="flex-1 h-2 bg-gray-200 dark:bg-black/50 rounded-full overflow-hidden shadow-inner border border-gray-300 dark:border-white/5">
                <div 
                  className="h-full bg-gradient-to-r from-amber-400 via-amber-500 to-yellow-400 dark:from-amber-600 dark:via-yellow-500 dark:to-amber-400 transition-all duration-1000 ease-out relative"
        ...
        <div className="sticky top-[53px] z-40 bg-slate-50/90 dark:bg-[#050505]/80 backdrop-blur-3xl pt-2 pb-3 px-4 mt-2 transition-colors duration-300">
          <div className="flex bg-gray-200 dark:bg-white/[0.04] p-1 rounded-xl border border-gray-300 dark:border-white/5 shadow-inner transition-colors duration-300">
            <Link 
              ...
              className={`flex-1 text-center py-2 text-[13px] font-bold rounded-lg transition-all duration-300 ${
                activeTab === 'gonderiler' ? 'bg-[#4DA3FF] text-black shadow-md' : 'text-gray-600 hover:text-gray-900 dark:text-gray-400 dark:hover:text-white'
              }`}
            >
              Gönderiler
            </Link>
            <Link 
              ...
              scroll={false} 
              className={`flex-1 text-center py-2 text-[13px] font-bold rounded-lg transition-all duration-300 ${
                activeTab === 'yorumlar' ? 'bg-[#4DA3FF] text-black shadow-md' : 'text-gray-600 hover:text-gray-900 dark:text-gray-400 dark:hover:text-white'
              }`}
            >
              Yorumlar
            </Link>
        ...
        <div className="pt-2 px-4">
          ...
          {activeTab === 'gonderiler' && (
            <div className="animate-in fade-in duration-300">
              {userPosts.length === 0 ? (
                <div className="text-center py-16 flex flex-col items-center justify-center bg-white dark:bg-white/[0.02] border border-gray-200 dark:border-white/5 shadow-sm dark:shadow-none rounded-3xl mt-2 transition-colors duration-300">
                  <div className="w-16 h-16 rounded-full bg-gray-50 dark:bg-white/5 border border-gray-100 dark:border-white/10 flex items-center justify-center mb-3">
                    <User size={28} className="text-gray-400 dark:text-gray-500" />
                  </div>
                  <p className="text-gray-500 dark:text-gray-400 font-medium text-[13px]">Henüz gönderi yok</p>
                </div>
              ) : (
            ...
            <div className="animate-in fade-in duration-300">
              {userComments.length === 0 ? (
                <div className="text-center py-16 flex flex-col items-center justify-center bg-white dark:bg-white/[0.02] border border-gray-200 dark:border-white/5 shadow-sm dark:shadow-none rounded-3xl mt-2 transition-colors duration-300">
                  ...
                  <p className="text-gray-500 dark:text-gray-400 font-medium text-[13px]">Henüz yorum yok</p>
                </div>
              ) : (
                <div className="space-y-3 mt-2">
                  {userComments.map((comment: any) => (
                    <div key={comment.id} className="bg-white dark:bg-white/[0.02] backdrop-blur-xl border border-gray-200 dark:border-white/[0.04] p-4 rounded-[24px] shadow-sm hover:shadow-md dark:hover:bg-white/[0.03] transition-all duration-300">
                      
                      <div className="flex justify-between items-start mb-2">
                        <div className="flex items-center gap-2">
                          <MessageCircle size={14} className="text-[#4DA3FF]" />
                          <span className="text-[11px] text-gray-500 dark:text-gray-400 font-bold tracking-wide">
                            {getRelativeTime(comment.createdAt)}
                          </span>
                        </div>
                        <div className="flex items-center gap-1.5 text-gray-600 dark:text-gray-400 text-[12px] font-bold bg-gray-50 dark:bg-white/5 px-2 py-1 rounded-full border border-gray-200 dark:border-white/5">
                          <Heart size={12} className={comment.likes > 0 ? "fill-pink-500 text-pink-500" : ""} /> {comment.likes || 0}
                        </div>
                      </div>
                      
                      <p className="text-gray-800 dark:text-gray-100 text-[14px] leading-relaxed mb-3 font-medium">
                        "{comment.content}"
                      </p>
                      
                      {comment.post && (
                        <div className="bg-gray-50 dark:bg-black/30 p-3 rounded-xl mb-3 border border-gray-200 dark:border-white/5 relative overflow-hidden transition-colors duration-300">
                          ...
                          <p className="text-gray-600 dark:text-gray-300 text-[12px] line-clamp-1 italic pl-1">
                      ...
                      <Link 
                        ...
                        className="inline-flex items-center gap-1.5 text-[#4DA3FF] text-[12px] font-bold hover:text-blue-500 dark:hover:text-blue-400 transition-colors"
                      >
